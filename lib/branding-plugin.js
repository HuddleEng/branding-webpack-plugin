...
function processImportedCss(imports){
    // sorts by resource, then gets only uniques
    return Array.from(new Set(imports.sort((a,b) => {
        const resourceSorted = [a.resource, b.resource].sort();
        return resourceSorted.indexOf(a.resource) - resourceSorted.indexOf(b.resource);
    }).map(({source}) => source)));
...
class BrandingPlugin {
    constructor(options = {}) {
        ...
        this.outputName = `${options.prefix ? `${options.prefix}.` : ''}[name].[hash].css`;
    }
    ...
    apply(compiler) {
        ...
        compiler.hooks.compilation.tap('BrandingPlugin', (compilation) => {
            compilation.hooks.optimizeChunkAssets.tapAsync('BrandingPlugin', (chunks, callback) => {
                ...
                const uniqueImportedCSS = processImportedCss(this.importedCSS); // other webpack plugins could be using branding plugin with the same css.
